...
test("v31 migration adds AgentDefinition bridge columns", async () => { const db = await openDb(); try { assert.equal(KIT_SQLITE_USER_VERSION, 31); assert.equal(db.pragma("user_version", { simple: true }), 31); assert.equal(kitSqliteHasAgentDefinitionBridge(db), true); } finally { db.close(); } });
