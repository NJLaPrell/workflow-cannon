...
test("v31 migration adds AgentDefinition bridge columns", async () => { const db = await openDb(); try { assert.equal(KIT_SQLITE_USER_VERSION, 32); assert.equal(db.pragma("user_version", { simple: true }), 32); assert.equal(kitSqliteHasAgentDefinitionBridge(db), true); } finally { db.close(); } });
