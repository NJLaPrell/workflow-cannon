...
test("registers orchestration, planning, and task-worker AgentDefinition fixtures", async () => { const db = await openDb(); const now = "2026-05-31T12:00:00.000Z"; try { for (const file of ["agent-definition-orchestration-agent.v1.json", "agent-definition-planning-agent.v1.json", "agent-definition-task-worker.v1.json"]) { const fixture = loadFixture(file); const parsed = parseAgentDefinitionInput({ agentDefinition: fixture }); assert.equal(parsed.ok, true, file); registerAgentDefinition(db, parsed.definition, now); } assert.equal(listAgentDefinitions(db, { orchestrationOnly: true }).length, 3); } finally { db.close(); } });
