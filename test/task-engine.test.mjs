...
// ---------------------------------------------------------------------------
// T186: TASKS.md generation
// ---------------------------------------------------------------------------

test("generateTasksMd produces markdown with status markers", () => {
  const tasks = [
    makeTask({ id: "T001", status: "ready", title: "First task", phase: "Phase 1", priority: "P1" }),
    makeTask({ id: "T002", status: "completed", title: "Done task", phase: "Phase 1" }),
    makeTask({ id: "T003", status: "in_progress", title: "Active task", phase: "Phase 2" })
  ];

  const md = generateTasksMd(tasks);
  assert.match(md, /# Workflow Cannon Tasks/);
  assert.match(md, /generated by the Task Engine/);
  assert.match(md, /\[ \] T001 First task/);
  assert.match(md, /\[x\] T002 Done task/);
  assert.match(md, /\[~\] T003 Active task/);
  assert.match(md, /## Phase 1/);
  assert.match(md, /## Phase 2/);
});

test("generateTasksMd includes dependencies and priority", () => {
  const tasks = [
    makeTask({
      id: "T001",
      status: "ready",
  ...
  assert.match(md, /Depends on: `T000`/);
  assert.match(md, /Unblocks: `T002`/);
});

test("generateTasksMd shows ready queue", () => {
  const tasks = [
    makeTask({ id: "T001", status: "ready", title: "A", phase: "P1", priority: "P2" }),
    makeTask({ id: "T002", status: "ready", title: "B", phase: "P1", priority: "P1" }),
    makeTask({ id: "T003", status: "completed", title: "C", phase: "P1" })
  ];

  const md = generateTasksMd(tasks);
  assert.match(md, /Ready queue:.*`T002`.*`T001`/);
});

// ---------------------------------------------------------------------------
// T186: Import from TASKS.md
// ---------------------------------------------------------------------------

test("importTasksFromMarkdown parses task entries", async () => {
  const workspace = await tmpDir();
  ...
  const t175 = result.tasks.find((t) => t.id === "T175");
  assert.equal(t175.status, "completed");
  assert.equal(t175.priority, "P1");
  assert.deepEqual(t175.dependsOn, []);
  assert.deepEqual(t175.unblocks, ["T176"]);
  assert.equal(t175.phase, "Phase 0");
  assert.ok(t175.technicalScope.length > 0);
  assert.ok(t175.acceptanceCriteria.length > 0);

  const t176 = result.tasks.find((t) => t.id === "T176");
  assert.equal(t176.status, "ready");
  assert.deepEqual(t176.dependsOn, ["T175"]);
});

test("importTasksFromMarkdown handles all status markers", async () => {
  const workspace = await tmpDir();
  const tasksPath = path.join(workspace, "TASKS.md");
  await writeFile(tasksPath, [
    "# Tasks",
  ...
  const result = await importTasksFromMarkdown(tasksPath);
  assert.equal(result.tasks.find((t) => t.id === "T001").status, "proposed");
  assert.equal(result.tasks.find((t) => t.id === "T002").status, "ready");
  assert.equal(result.tasks.find((t) => t.id === "T003").status, "in_progress");
  assert.equal(result.tasks.find((t) => t.id === "T004").status, "blocked");
...
test("importTasksFromMarkdown throws for missing file", async () => {
  await assert.rejects(
    ...
    (err) => err instanceof TaskEngineError && err.code === "import-parse-error"
  );
});

// ---------------------------------------------------------------------------
// T186: Import round-trip (import → generate produces equivalent output)
// ---------------------------------------------------------------------------

test("import then generate produces parseable output", async () => {
  const workspace = await tmpDir();
  const tasksPath = path.join(workspace, "TASKS.md");
  await writeFile(tasksPath, [
    "# Tasks",
  ...
  const importResult = await importTasksFromMarkdown(tasksPath);
  const generatedMd = generateTasksMd(importResult.tasks);
...
test("taskEngineModule registration includes all instruction entries", () => {
  ...
  assert.ok(names.includes("import-tasks"));
  assert.ok(names.includes("generate-tasks-md"));
...
test("taskEngineModule onCommand import-tasks and generate-tasks-md round-trip", async () => {
  const workspace = await tmpDir();
  const docsDir = path.join(workspace, "docs", "maintainers");
  await mkdir(docsDir, { recursive: true });
  await writeFile(path.join(docsDir, "TASKS.md"), [
    "# Tasks",
    "",
    "## Phase 0",
    "",
  ...
  const importResult = await taskEngineModule.onCommand(
    { name: "import-tasks", args: {} },
    ctx
  );
  assert.equal(importResult.ok, true);
  assert.equal(importResult.data.imported, 2);

  const listResult = await taskEngineModule.onCommand(
  ...
  const outputPath = path.join(workspace, "output", "TASKS.md");
  const genResult = await taskEngineModule.onCommand(
    { name: "generate-tasks-md", args: { outputPath: "output/TASKS.md" } },
    ctx
  );
  assert.equal(genResult.ok, true);
  const generated = await readFile(outputPath, "utf8");
  assert.match(generated, /T175/);
  assert.match(generated, /T176/);
});
