...
**Role, phase, and deliver:** The dashboard shows **Current phase** / **Next phase** directly below the WC Agent status banner, with **Role** / **Agent temperament** / **Presentation** in the Status tab profile card (from **`dashboard-summary`**). **Deliver** is enabled when the current phase bucket has at least one **`ready`** execution task; hover shows the count. Refresh runs **`dashboard-summary`** only.
