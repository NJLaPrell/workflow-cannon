...
export class GuidancePanel {
  ...
  open(): void {
    ...
    panel.webview.onDidReceiveMessage((msg) => {
      ...
      if (msg?.type === "activationAction") void this.reportAction(String(msg.action ?? ""), String(msg.activationId ?? ""));
  ...
  private async runActivationMutation(command: string, rawPayload: unknown): Promise<void> {
    ...
    const note = typeof payload.note === "string" && payload.note.trim() ? payload.note.trim() : "Guidance activation editor";
    const actor = this.defaultActorFromEnvironment() ?? "dashboard";
    const detail = [`Command: ${command}`, `Actor: ${actor}`, `Activation: ${activationId}`, `Note: ${note}`].join("\n");
    const confirmed = await vscode.window.showWarningMessage("Save draft CAE activation?", { modal: true, detail }, "Save draft");
    if (confirmed !== "Save draft") {
    ...
    try {
      ...
      await this.panel?.webview.postMessage({
        type: "actionResult",
        ok: result.ok === true,
        text: result.ok === true ? `${command} completed${warnings ? ` with ${warnings} warning(s).` : "."}` : String(result.message ?? result.code ?? `${command} failed.`)
      });
      if (result.ok === true) await this.runRefresh();
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
