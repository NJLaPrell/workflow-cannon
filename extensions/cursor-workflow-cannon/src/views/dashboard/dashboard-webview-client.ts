...
export function buildDashboardWebviewBootstrapScript(embeddedCaeBootstrapSource: string): string {
  ...
  if (rootEl) rootEl.addEventListener('click', function(ev) {
    ...
    if (act === 'plan-artifact-cancel') {
      var cancelPlanId=(t.getAttribute('data-plan-id')||'').trim();
      var cancelPlanRef=(t.getAttribute('data-plan-ref')||'').trim();
      var cancelIdeaId=(t.getAttribute('data-idea-id')||'').trim();
      var cancelCard=t.closest('[data-wc-plan-card-id]');
      var cancelTitle='';
      if (cancelCard) {
        var cancelSummary=cancelCard.querySelector('.wc-plan-card-rollup-summary');
        cancelTitle=cancelSummary ? String(cancelSummary.textContent||'').trim() : '';
      }
      if (cancelPlanId && cancelPlanRef) {
        vscode.postMessage({
          type: 'cancelPlanArtifact',
          planId: cancelPlanId,
          planRef: cancelPlanRef,
          ideaId: cancelIdeaId,
          title: cancelTitle
        });
      } else {
        vscode.postMessage({ type: 'invalidPlanArtifactAction', action: 'cancel', reason: 'missing-plan-identity' });
      }
      return;
    }
    if (act === 'plan-artifact-delete') {
      var delPlanId=(t.getAttribute('data-plan-id')||'').trim();
      var delPlanRef=(t.getAttribute('data-plan-ref')||'').trim();
      var delIdeaId=(t.getAttribute('data-idea-id')||'').trim();
      var delCard=t.closest('[data-wc-plan-card-id]');
      var delTitle='';
      if (delCard) {
        var delSummary=delCard.querySelector('.wc-plan-card-rollup-summary');
        delTitle=delSummary ? String(delSummary.textContent||'').trim() : '';
      }
      if (delPlanId && delPlanRef) {
        vscode.postMessage({
          type: 'deletePlanArtifact',
          planId: delPlanId,
          planRef: delPlanRef,
          ideaId: delIdeaId,
          title: delTitle
        ...
        vscode.postMessage({ type: 'invalidPlanArtifactAction', action: 'delete', reason: 'missing-plan-identity' });
      }
      return;
    }
