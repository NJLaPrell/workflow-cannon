...
**CLI (source of truth for persisted profiles):** use **`pnpm exec wk run`** with the **agent-behavior** module commands (discover exact names via **`pnpm exec wk run`** with no subcommand). Follow **`src/modules/agent-behavior/README.md`** and **`.ai/AGENT-CLI-MAP.md`** for tiers and JSON shapes.
