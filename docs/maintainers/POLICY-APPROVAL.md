...
**`workspace-kit run` does not read `WORKSPACE_KIT_POLICY_APPROVAL`** for the run path itself. Repo helpers **`scripts/run-transcript-cli.mjs`** (`pnpm run transcript:ingest`) and **`scripts/pre-release-transcript-hook.mjs`** parse that env var and pass **`policyApproval`** inside the **third JSON argument** to `ingest-transcripts` so headless flows match maintainer intent. The Phase 55 reference runner **`tools/github-invocation/run-github-delivery.mjs`** reads **`WORKSPACE_KIT_GITHUB_RUN_POLICY_APPROVAL`** only to synthesize missing **`policyApproval`** on **`WORKSPACE_KIT_GITHUB_RUN_ARGS_JSON`** for mutating routes — it still does **not** treat GitHub comments as approval; see **`docs/maintainers/runbooks/github-workflow-cannon-invocation.md`**. Other package scripts that wrap `run` must pass JSON **`policyApproval`**—or invoke `init`/`config`/`upgrade`, which do read **`WORKSPACE_KIT_POLICY_APPROVAL`**.
