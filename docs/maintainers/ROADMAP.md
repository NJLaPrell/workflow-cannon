...
- **Phase 28 (maintainer and agent operability)** is **IN PROGRESS** in task-engine: **`T392`**, **`T443`–`T449`** are **`ready`** with phase label **Phase 28 (maintainer and agent operability)** and explicit **`dependsOn`** (queue-health **`T443`** → canonical phase **`T444`** → structured phase **`T445`**; **`list-tasks`** hints **`T446`**; **AGENT-CLI-MAP** queue pattern **`T447`** depends on **`T443`**+**`T446`**; **`wk`** bin **`T448`** → repo CLI reference audit **`T449`**; **`T392`** depends on completed **`T389`**/**`T390`**).
