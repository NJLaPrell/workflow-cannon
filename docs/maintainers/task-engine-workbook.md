...
| `import-parse-error` | Failed to parse TASKS.md during import | import-tasks command |
...
| `import-tasks` | `{ sourcePath?: string }` | `{ imported: number, skipped: number, errors: string[] }` | One-time import from TASKS.md |
| `generate-tasks-md` | `{ outputPath?: string }` | `{ outputPath: string, taskCount: number }` | Generate read-only TASKS.md from engine state |
| `get-next-actions` | `{}` | `NextActionSuggestion` | Priority-sorted ready queue with blocking chain analysis |

---

## 9. Generated TASKS.md

The engine produces a read-only `docs/maintainers/TASKS.md` from state.

### Format

Matches the current TASKS.md section structure:
...
- Current phase in execution: _Phase N (description)._
- Ready queue: `T###, T###`
- Execution order: `T### → T### → T###`

## Phase N section title
...
One-time parser that reads the current `docs/maintainers/TASKS.md` and imports tasks into `.workspace-kit/tasks/state.json`.

**Parsing rules:**

- Status markers map: `[p]` → `proposed`, `[ ]` → `ready`, `[~]` → `in_progress`, `[!]` → `blocked`, `[x]` → `completed`, `[-]` → `cancelled`
- Task ID extracted from heading: `### [marker] T### Title`
- Priority extracted from `- Priority: P#` line
- Dependencies extracted from `- Depends on:` line (comma-separated task IDs, `none` means empty array)
- Unblocks extracted from `- Unblocks:` line
- Phase grouping extracted from `## Phase N` section heading
- Technical scope extracted as list items under `- Technical scope:`
- Acceptance criteria extracted as list items under `- Acceptance criteria:`
- Approach extracted from `- Approach:` line

**Post-import:** TASKS.md becomes a generated view. The engine's JSON state is the source of truth.
