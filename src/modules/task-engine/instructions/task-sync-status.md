<!--
agentCapsule|v=1|command=task-sync-status|module=task-engine|schema_only=pnpm exec wk run task-sync-status --schema-only '{}'
-->

# task-sync-status

Read-only comparison of the local SQLite task-state projection vs the canonical **`workflow-cannon/task-state`** git branch.

## Usage

```
pnpm exec wk run task-sync-status '{}'
pnpm exec wk run task-sync-status '{"fetch":true}'
```

## Arguments

| Field | Type | Required | Description |
| --- | --- | --- | --- |
| `fetch` | boolean | no | When **true**, runs `git fetch origin <branch>` before resolving the ref. |
| `branch` | string | no | Override branch name (default `workflow-cannon/task-state`). |

## Returns

- `data.syncState`: **`current`**, **`behind`**, **`missing`**, or **`conflict`**
- `data.remoteLatestSequence`, `data.localAppliedSequence`, `data.gitRef`, `data.remoteTipSha`
- `data.localProjection`: **`fresh`**, **`behind`**, **`conflict`**, **`rebuilding`**, or **`offline`**
- `data.outbox`: queue counters (`pending`, `publishing`, `failed`, `conflict`) plus `oldestPendingAgeMs`
- `data.remote`: branch posture (`branch`, `behind`, `remoteLatestSequence`, `remoteTipSha`, `lastPublishedAt`)
- `data.recommendedAction`: **`none`**, **`wait`**, **`hydrate`**, **`resolve-conflict`**, or **`run-publish`**

## Notes

- Does **not** mutate SQLite or JSONL — safe for dashboard polling when not passing `fetch:true`.
- Recovery alias: **`task-state-status`** (same argv and policy; prefer **`task-sync-status`** for new scripts).
