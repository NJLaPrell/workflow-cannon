...
| `agentStatus` | `{ schemaVersion: 1, source: "derived", kind, label, confidence, updatedAt, taskId?, phaseKey?, command?, prNumber?, version?, detail? }`. Derived kinds are `unavailable`, `planning`, `blocked`, `working_task`, `delegating_task`, `ready_task`, and `awaiting_instruction`. Precedence is unavailable/error, active planning, blocked current work, in-progress task, active team/subagent delegation, suggested next ready task, then `Awaiting Instruction`. Derived status is an observable hint, not proof of live agent intent. |
