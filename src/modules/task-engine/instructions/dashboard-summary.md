...
| `includePhaseKickoff` | boolean | `false` | When `true`, adds **`phaseKickoff`** — bounded kickoff audit for **`phaseKey`** (defaults to workspace current phase) using **`phase-kickoff-readiness`** semantics. |
| `phaseKey` | string | workspace current | Phase scope for **`phaseFocus`** / **`phaseKickoff`** when those includes are set. |
...
`buildDashboardBase` assembles shared dashboard state with projection guards (`dashboardSummaryNeedsQueueRollups`, `dashboardSummaryNeedsStatusRollups`, `dashboardSummaryNeedsAgentActivityRollups`, etc.) so expensive queue rollups and overview-only skips (e.g. planning session file read) happen **before** projection assembly. Section builders (`buildDashboardOverviewProjection`, `buildDashboardQueueProjection`, `buildDashboardStatusProjection`, `buildDashboardAgentActivityProjection`, `buildDashboardFullProjection`) map the base object; `finalizeDashboardSummaryProjection` stamps `dashboardProjection` and applies field omissions for each slice.
...
| `dashboardProjection` | When set, names the section slice (`full`, `overview`, `queue`, `status`, `agentActivity`). Omitted or **`full`** on default aggregate responses. |
...
| `phaseKickoff` | Optional kickoff audit summary when **`includePhaseKickoff`** is true — `passed`, `findingCount`, `enforcementMode`, top `findings[]` for the scoped **`phaseKey`** |
...
| `agentActivitySummary` | `{ schemaVersion: 1, generatedAt, source, activeCount, staleCount, needsAttentionCount, main, active[], needsAttention[], inferredFallback, sourceMap }` — activity projection built from live leases, assignments, and subagent sessions. Open subagent sessions are coordination rows supplied by the registry; stale external sessions must be closed with `close-subagent-session` or they can remain visible in the dashboard agent card. `agentActivity` refreshers can request this slice without queue/status rollups; `full` still includes it for compatibility. |
