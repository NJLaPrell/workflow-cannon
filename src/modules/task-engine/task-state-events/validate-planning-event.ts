...
import eventSchema from "./schemas/planning-state-event.v1.json" with { type: "json" };
...
export function validatePlanningStateEvent(
  input: unknown
): { ok: true; data: PlanningStateEventV1 } | { ok: false; errors: string[] } {
  const validate = loadValidator();
  if (!validate(input)) {
    return { ok: false, errors: formatAjvErrors(validate.errors) };
  }
  return { ok: true, data: input as PlanningStateEventV1 };
}
