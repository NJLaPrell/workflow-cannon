...
import type { TaskEntity, TaskStatus, TransitionEvidence, TransitionGuard } from "./types.js";
import {
  TaskEngineError,
  TransitionValidator,
  getTransitionAction,
...
export class TransitionService {
  ...
  async runTransition(request: TransitionRequest): Promise<TransitionResult> {
    ...
    if (!validation.allowed) {
      ...
      if (rejection?.code === "dependency-unsatisfied") {
        throw new TaskEngineError(
          ...
          rejection.message ?? "Dependencies not satisfied"
        );
      }
    ...
    const autoUnblockResults =
      targetState === "completed" ? this.autoUnblock(request.taskId, timestamp, request.actor) : [];
