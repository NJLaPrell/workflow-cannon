...
- `persistenceBackend`: `json` (default) or `sqlite`. When `sqlite`, the **task** document is read/written from `sqliteDatabaseRelativePath` (`task_store_json`; legacy installs may still have a second wishlist column until **`migrate-wishlist-intake`** runs).
