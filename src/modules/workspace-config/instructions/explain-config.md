...
- `facet` (optional): bounded facet id — `tasks`, `planning`, `improvement`, `kit`, `modules`, `policy`, `agentPresentation`, `responseTemplates`. Returns `facet`, `facetKeys`, `entries[]` (each entry includes `path` plus the same fields as a single-path explain), and `count`
