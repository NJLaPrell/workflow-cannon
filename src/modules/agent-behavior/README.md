...
**Interview (`interview-behavior-profile`):** Uses gitignored **`.workspace-kit/agent-behavior/interview-session.json`** only. It does **not** participate in task-engine **`planningGeneration`** / optimistic concurrency; no `expectedPlanningGeneration` on this command unless that contract changes in a future phase.
