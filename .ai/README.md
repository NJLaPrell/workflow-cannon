...
chat_feature|id=CF005|title=Plan from Ideas toward execution tasks|summary=Turn loose Ideas into PlanArtifact drafts and phased execution work without losing the planning context that made them worth keeping.|steps=Pick an Ideas row or describe the idea in chat>Attach `.ai/playbooks/planner-chat.md` or enable planner-chat guidance>Follow the playbook for draft/review/accept flows and `finalize-plan-to-phase` when materializing tasks
