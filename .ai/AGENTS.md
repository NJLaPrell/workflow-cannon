...
ref|id=roadmap|target=.ai/WORKSPACE-KIT-SESSION.md|type=file|status=active
...
rule|id=A010|level=must|scope=agent|directive=keep_strategy_in_roadmap_execution_in_task_engine_state_and_release_ops_in_releasing|risk=medium|approval=none|override=warn|status=active|refs=.ai/WORKSPACE-KIT-SESSION.md,.workspace-kit/tasks/workspace-kit.db,.workspace-kit/tasks/state.json,.ai/machine-cli-policy.md|why=rationale_for_A010
...
rule|id=A029|level=must|scope=agent|directive=when_triaging_improvement_backlog_to_ready_queue_follow_playbook_improvement_triage_top_three_attach_or_requestable_rule|risk=medium|approval=none|override=warn|status=active|refs=.ai/MACHINE-PLAYBOOKS.md,.cursor/rules/playbook-improvement-triage-top-three.mdc|why=bounded_promotion_and_evidence_backed_accept
